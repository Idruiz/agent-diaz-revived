...
export function diagramSvg(d:{title:string;nodes:string[];caption?:string},width=1000,height=420):string{
 const gap=(width-140)/d.nodes.length;let body="";d.nodes.forEach((n,i)=>{const x=70+i*gap,y=150;body+=`<rect x="${x}" y="${y}" width="${gap-35}" height="100" rx="14" fill="${i%2?'#e7eef2':'#f1e5c5'}" stroke="#c99a2e"/><text x="${x+(gap-35)/2}" y="${y+56}" text-anchor="middle" class="node">${esc(n.slice(0,28))}</text>`;if(i<d.nodes.length-1)body+=`<path d="M${x+gap-35},${y+50} L${x+gap-5},${y+50}" stroke="#2f739c" stroke-width="4" marker-end="url(#a)"/>`});return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><defs><marker id="a" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#2f739c"/></marker></defs><style>text{font-family:Arial;fill:#17324d}.title{font-size:28px;font-weight:bold}.node{font-size:16px;font-weight:bold}.cap{font-size:14px;fill:#667784}</style><rect width="100%" height="100%" fill="#fff"/><text x="60" y="55" class="title">${esc(d.title)}</text>${body}<text x="60" y="365" class="cap">${esc(d.caption||"")}</text></svg>`}
