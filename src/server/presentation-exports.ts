...
function isFreshFile(target: string, sourceMtimeMs: number, minimumSize: number): boolean {
  if (!fs.existsSync(target)) return false;
  const stat = fs.statSync(target);
  return stat.isFile() && stat.size >= minimumSize && stat.mtimeMs >= sourceMtimeMs;
}

function assertPdf(bytes: Buffer): void {
  if (bytes.length < 2_000 || bytes.subarray(0, 5).toString("ascii") !== "%PDF-")
...
export function browserPresentationHtml(title: string, pdfBytes: Buffer): string {
  assertPdf(pdfBytes);
...
async function renderPdf(
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "diaz-presentation-export-"));
...
export async function ensurePresentationExports(
...
): Promise<{ html: PresentationExportFile; pdf: PresentationExportFile }> {
  if (!isPresentationArtifact(artifact))
    throw new Error("Only validated PPTX artifacts can be exported as presentation companions");
  const expectedSource = safeJoin(config.artifactDir, artifact.name);
  if (path.resolve(artifact.path) !== expectedSource || !fs.existsSync(expectedSource))
    throw new Error("Presentation source bytes are missing or outside the artifact directory");
  ...
  if (!isFreshFile(pdfTarget, sourceStat.mtimeMs, 2_000)) {
    await renderPdf(config, expectedSource, pdfTarget);
  ...
  if (!isFreshFile(htmlTarget, pdfStat.mtimeMs, 3_000)) {
    ...
    if (!html.startsWith("<!doctype html>") || !html.includes('id="pdf-data"'))
      throw new Error("Generated browser presentation failed deterministic HTML validation");
...
export function presentationArtifactViews(
  ...
  for (const artifact of artifacts) {
    ...
    for (const format of ["html", "pdf"] as const) {
      ...
      const stat = fs.statSync(target);
      if (!stat.isFile() || stat.size < (format === "pdf" ? 2_000 : 3_000)) continue;
      expanded.push({
        ...artifact,
...
export function presentationExportRoutes(
  ...
  router.get(
    ...
    async (req, res, next) => {
      ...
      res.json({
        ...job,
        artifacts: presentationArtifactViews(config, db.listArtifacts(job.id)),
  ...
  router.get(
    ...
    auth.requireAuth,
    async (req, res, next) => {
      const match = req.params.id.match(VIRTUAL_ID);
      ...
      if (!artifact || !isPresentationArtifact(artifact))
        return res.status(404).json({ error: "Presentation artifact not found" });
      try {
